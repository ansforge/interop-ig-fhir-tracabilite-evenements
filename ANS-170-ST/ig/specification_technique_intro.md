...
* D’autre part, au protocole et à la structure de messages syslog, éventuellement couplée à la structure de message d’audit définie par [DICOM](http://dicom.nema.org/dicom/2013/output/chtml/part15/sect_A.5.html). La syntaxe utilisée pour les exemples HL7 FHIR de ces spécifications est la syntaxe JSON.

### Profil IHE utilisé

Le profil utilisé est le profil IHE ATNA qui offre les fonctionnalités suivantes :

* Enregistrement de traces dans un dépôt via la transaction ITI-20 *Soit à l’aide du protocole Syslog (c’est-à-dire conforme à la RFC5424) *Soit à l’aide de l’opération « Create » de l’API REST de FHIR ou au format HL7 FHIR (ressource FHIR AuditEvent) ;
